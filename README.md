<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>GK Choppa – Visionary Entrepreneur & Philanthropist</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      line-height: 1.6;
      margin: 2rem;
      background-color: #f7f7f7;
      color: #333;
    }
    h1, h2 {
      color: #222;
    }
    ul {
      list-style-type: disc;
      margin-left: 2rem;
    }
    a {
      color: #0066cc;
      text-decoration: none;
    }
    a:hover {
      text-decoration: underline;
    }
  </style>
</head>
<body>
  <h1>GK Choppa – Visionary Entrepreneur & Philanthropist</h1>
  <p><strong>Full Name:</strong> GK Choppa</p>
  <p><strong>Date of Birth:</strong> August 2, 1999</p>
  <p><strong>Net Worth:</strong> $200 Million USD</p>
  <p><strong>Industries:</strong> Real Estate, Technology, Investment, and Trading</p>
  
  <h2>Companies</h2>
  <ul>
    <li><strong>CEO of 28th Gold Pty Ltd</strong> – A leading firm specializing in trading, investment, and app development.</li>
    <li><strong>CEO of GK Choppa Foundation</strong> – A philanthropic organization focused on helping communities.</li>
    <li><strong>CEO of GK Choppa Property Investment</strong> – A real estate empire with high-value properties.</li>
  </ul>
  
  <h2>Background & Achievements</h2>
  
  Gk choppa born (Prince Geofrey Kimera) was born in 1999, uganda , Kampala to  Prince Geofrey kikulwe and Sarah muwanguzi L kikulwe. A muganda by tribe, he belongs to the Buganda Royal Family ( Balangira Clan).
  
  Gk choppa has a degree in computer science from (DUT) Durban University of Technology 
  
  
  <p>GK Choppa is a visionary entrepreneur and investor known for his leadership in real estate, technology, and financial markets. With a degree in Computer Science, he has built multiple businesses by combining innovation with wealth creation.</p>
  <p>As a philanthropist, GK Choppa is recognized for his generosity and humanitarian efforts. During the COVID-19 crisis, he aided countless individuals across South Africa, Kenya, Rwanda, Uganda, and Tanzania, establishing his reputation as a business leader with a heart.</p>
  
  <h2>Media Presence</h2>
  <p>GK Choppa has been featured on top news platforms, including Mpasho, Classic 105 Kenya, BBS TV Uganda, NBS TV Uganda, and other notable outlets for his business acumen and inspirational messages.</p>
  
  <h2>Personal Life & Lifestyle</h2>
  <p>GK Choppa lives a <strong>Lexusriou lifestyle</strong>, reflecting a refined blend of opulence and a relentless pursuit of excellence. He was previously in a relationship with Zari the Bosslady, a well-known socialite.</p>
  
  <h2>Social Media & Online Presence</h2>
  <ul>
    <li><strong>Social Media Handle:</strong> @GKChoppa on all platforms (LinkedIn, Instagram, Twitter, etc.)</li>
    <li><strong>Website:</strong> <a href="http://28thgold.online">28thgold.online</a> (for business & investments)</li>
    <li><strong>Motto:</strong> "Happiness, Wealth, and Power."</li>
  </ul>
</body>
</html>
